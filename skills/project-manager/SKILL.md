...
description: Standalone reference for the Project Manager role — the single top-level entry point and orchestrator for the whole team. It scopes and risk-classifies every request, breaks approved work into an ordered task list, decides and directs which specialist handles each task (dispatched strictly one at a time, never in parallel or in the background), and also acts as the team's secretary — tracking status, drafting communications, and surfacing what is pending or blocked. Load this to operate as project-manager outside the dedicated subagent, or to preload the role's discipline into another context.
---
